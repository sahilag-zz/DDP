// choosing splitting variables by which occurs the most in current cnf
...
int mySolver::splitVar(vector< vector<int> > &tempcnf){
	int freq[nvars],maxindex=1,max=0;
	for (int i=0;i<nvars;i++) freq[i]=0;
	for (int i=0;i<tempcnf.size();i++){
		for (int j=0;j<tempcnf[i].size();j++){
			freq[abs(tempcnf[i][j])-1]++;
		}		
	}
	for (int i=0;i<nvars;i++){
		if(freq[i]>max){max=freq[i]; maxindex = i;}
	}
	return maxindex+1;
}
...
// doesnt check if cnf is UNSAT

satbool mySolver::simple_simplify(vector<int> assums, vector< vector<int> > &tempcnf){
...
void mySolver::gencnf(vector< vector<int> > &tempcnf){
	static int count = 0;
	ofstream ofile;
	ostringstream oss;
	oss << "gen" << count << ".cnf" ;
