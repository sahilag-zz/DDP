...
satbool mySolver::decompose(set<int> assums){
	...
	//returning UNSAT because we want cnfs to be generated for all branches of 
	//the tree
	if (tempcnf.size()<nclauses-10) {count--; gencnf(tempcnf,assums); return UNSAT;}
...
// ------------------------- SolveMinisat() ----------------------------------//
void mySolver::gencnf(vector< vector<int> > &tempcnf, set <int> assums){
